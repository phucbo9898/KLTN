...
<div class="row">
  <div class="col-md-8 pl-3 pr-3">
    <div class="form-group">
      ...
      <input type="text" class="form-control" name="name" value="{{old('name',isset($product)?$product->name:"")}}" placeholder="Nhập tên sản phẩm...">
    </div>
    ...
    <div class="form-group">
      ...
      <select name="category_id" id="select_category_id" class="form-control">
        ...
        @foreach($categories as $key => $category)
          <option value="{{$category->id}}" {{old('category_id',isset($product->category_id)?(($product->category_id==$category->id)?"selected":""):"")}}>{{$category->name}}</option>
        @endforeach
      </select>
    </div>
    <div class="form-group">
      <label>Giá sản phẩm: </label>
      <input type="number" class="form-control" name="price" value="{{old('price',isset($product->price)?$product->price:"")}}" placeholder="Nhập giá sản phẩm...">
    </div>
    <div class="form-group">
      <label>Giảm giá: </label>
      <input type="number" class="form-control" name="sale" value="{{old('sale',isset($product->sale)?$product->sale:"")}}" placeholder="Giảm giá sản phẩm...">
    </div>
    <div class="form-group">
      <label>Ảnh minh họa:</label>
      @if(isset($product->image))
          <img  id="img_output" style="width:240px;height:180px; margin-bottom:10px" src="{{asset('upload/pro_image/'.$product->image)}}"/>
      @else
