    ...
    <div class="col-md-8 pl-3 pr-3">
        ...
        <div class="form-group">
            ...
            <textarea name="content" id="ckeditor" rows="5" class="form-control"
                      placeholder="Nhập nội dung sản phẩm...">{{old('content',isset($product->content)?$product->content:"")}}</textarea>
        </div>
        ...
        <div class="form-group">
            ...
            <select name="category_id" id="select_category_id" class="form-control">
                ...
                @foreach($categories as $key => $category)
                    <option
                        value="{{$category->id}}" {{old('category_id',isset($product->category_id)?(($product->category_id==$category->id)?"selected":""):"")}}>{{$category->name}}</option>
                @endforeach
            </select>
        </div>
        <div class="form-group">
